#include "my_utils/IO/comm_udp.hpp"
...
void send_data(int& _socket, int port, void* data, int data_size,
               const char* ip_addr) {
    int slen;
    if (_socket == 0) {
    ...
    int ret = sendto(_socket, data, data_size, 0,
                     (const struct sockaddr*)&si_to, sizeof(si_to));
}

void receive_data(int& _socket, int port, void* data, int data_size,
                  const char* ip_addr) {
    ...
    int bindresult =
        bind(_socket, (const sockaddr*)&si_me, (socklen_t)sizeof(si_me));
...
void receive_data_unix(const char* server_name, void* data, int data_size) {
    ...
    client_socket =
        accept(server_socket, (sockaddr*)&client_addr, (socklen_t*)&client_len);
    if (client_socket == -1) {
        perror("accept");
    } else {
...
void send_data_unix(const char* server_name, void* data, int data_size) {
    ...
    if (sock < 0) {
        printf("[client socket] fail to open socket\n");
    }
