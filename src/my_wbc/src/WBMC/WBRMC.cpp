...
#include <my_wbc/WBMC/WBRMC.hpp>
#include <my_utils/IO/IOUtilities.hpp>

WBRMC::WBRMC(const std::vector<bool>& act_list, const Eigen::MatrixXd* Jci)
    : WBC(act_list, Jci) {
    my_utils::pretty_constructor(3, "WBRMC");
...
void WBRMC::updateSetting(const Eigen::MatrixXd& A, const Eigen::MatrixXd& Ainv,
...
void WBRMC::setTorqueLimits(const Eigen::VectorXd &_tau_min,
                            const Eigen::VectorXd &_tau_max) {
    tau_min_ = _tau_min;
    tau_max_ = _tau_max;
}

void WBRMC::makeTorqueGivenRef(const Eigen::VectorXd& des_jacc_cmd,
                           const std::vector<ContactSpec*>& contact_list,
                           Eigen::VectorXd& cmd, void* extra_input) {
    if (!b_updatesetting_) {
        printf("[Warning] WBRMC setting is not done\n");
    }
    if (extra_input) data_ = static_cast<WBRMC_ExtraData*>(extra_input);
    ...
    if(des_jacc_cmd.size() == num_act_joint_){
        ...
        std::cout << " dim of des_jacc_cmd is wrong!!! @ WBRMC" << std::endl;
        exit(0);
    }
...
void WBRMC::_Build_Inequality_Constraint() {
...
void WBRMC::_Build_Equality_Constraint() {
...
void WBRMC::_BuildContactMtxVect(const std::vector<ContactSpec*>& contact_list) {
...
void WBRMC::_OptimizationPreparation(const Eigen::MatrixXd& Aeq,
...
void WBRMC::_GetSolution(Eigen::VectorXd& cmd) {
    ...
    //0112 my_utils::saveVector(data_->Fr_, "Fr_WBRMC");
