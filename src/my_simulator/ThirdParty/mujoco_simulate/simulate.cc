...
const mjuiDef defFile[] = {
  ...
  {mjITEM_BUTTON,    "Motion Script",      2, nullptr,                    ""},
...
void uiEvent(mjuiState* state) {
  ...
  if ((state->dragrect==sim->ui0.rectid) ||
    ...
    if (it && it->sectionid==SECT_FILE) {
      switch (it->itemid) {
      case 0:             // Motion Script
        sim->motionrequest.store(1);
        std::cout<<" motionrequest pressed "<<std::endl;
        break;
      case 1:             // Quit
  ...
  if (state->type==mjEVENT_KEY && state->key!=0) {
    ...
    case '-':                   // slow down
      {
        int numclicks = sizeof(mj::Simulate::percentRealTime) / sizeof(mj::Simulate::percentRealTime[0]);
...
void Simulate::render() {
  ...
  float desiredRealtime = mj::Simulate::percentRealTime[this->realTimeIndex];
