...
// Needs boundary conditions x(to), xdot(to), xddot(to), x(tf), xdot(tf),
// xddot(tf) 		where to and tf are the initial and final times respectively. Also,
// tf > to.
...
class MinJerk_OneDimension {
   public:
    ...
    MinJerk_OneDimension(const Eigen::Vector3d& init,
